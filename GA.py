...
class Chromosome:
    ...
    def update_fitness(self,value,domain,fitness_func):
        self.fitness=fitness_func(map_binary_value_to_domain(bin_num=value,domain_max=max(domain),domain_min=min(domain),bin_size=len(self.genes)))
...
class Genetic:
    ...
    def main(self):
        ...
        for i in range(self.number_of_papulation):
            ...
            fitness=new_chromosome.get_chromosome_value()
            new_chromosome.update_fitness(fitness,domain=self.domain,fitness_func=self.fitness_function)
            first_generation.add_chromosome_to(new_chromosome)

        for item in first_generation.members:
            print(item.get_chromosome_value())
        ...
        while counter != 0:
            ...
            for i in range(int(self.number_of_papulation/2)):
                ...
                if self._crossover_check():
                    ...
                    child1.update_fitness(value=child1.get_chromosome_value(),domain=self.domain,fitness_func=self.fitness_function)
                    child2=Chromosome()
                    child2.genes=final_children[1]
                    child2.update_fitness(value=child2.get_chromosome_value(),domain=self.domain,fitness_func=self.fitness_function)
